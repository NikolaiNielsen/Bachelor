...
\begin{document}
	...
	\subsection{Examples}
	Say we want to simulate neutron scattering on a cubic lattice with a two atom basis (one at lattice points, the other at the centre of the unit cell). We set the relative scattering lengths as 1 and 0.5 respectively, and we set $ \V{k}_{in} = (0, 0, -1.5)$ in units of $ 2\pi/a $. This is done by the following line, and produces figure \textbf{FIGREF}
\begin{lstlisting}
...
\end{lstlisting}
	...
	If instead we want to plot scattering on a BCC lattice, with the same wave vector, we just set the scattering length for both atoms to 1. If we further want to highlight the scattering corresponding to the Miller indices (112) we write the following, which gives \textbf{FIGUREREF}
\begin{lstlisting}
Scattering(k_in=np.array([0, 0, -1.5])
		   basis=np.array([[0, 0, 0], [0.5, 0.5, 0.5]]),	
		   ...
		   highlight=(1,1,2))
\end{lstlisting}
	As a last example, say we just want to plot the family of lattice planes
\end{document}