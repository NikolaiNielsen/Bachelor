...
\begin{document}
	...
	\begin{wrapfigure}{r}{2in}
		\begin{center}
			\includegraphics[width=\linewidth]{figures/lattice_demo_1.pdf}
		\end{center}
		\caption{An attempt to plot eight conventional unit cells for an FCC lattice, by using primitive lattice vectors with coefficients in the set $ \{0, 1, 2\} $. This does not fill out all eight unit cells, so we need to plot more lattice points than this. In practise we need $ \{-2, -1, 0, 1, 2, 3, 4\} $}
		\label{fig:lattice_demo}
	\end{wrapfigure}
	...
	\subsection{Example}
	As an example, we plot a simple cubic lattice with a two atom basis (corresponding to the basis of a BCC lattice with conventional unit cells), where the atoms on the lattice points are grey and the body centred atoms are blue (figure \ref{fig:lattice_demo}), we write the following:
\begin{lstlisting}
